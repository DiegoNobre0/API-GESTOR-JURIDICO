...
var userCreateSchema = z.object({
  ...
  senha: z.string().min(6, "A senha deve ter no m\xEDnimo 6 caracteres"),
...
var loginSchema = z2.object({
  ...
  senha: z2.string().min(6, "A senha deve ter no m\xEDnimo 6 caracteres")
});
...
var createProcessoSchema = z3.object({
  // --- DADOS DO CLIENTE ---
  clienteNome: z3.string().min(3, "Nome do cliente \xE9 obrigat\xF3rio"),
  // NOVO: Telefone é obrigatório para fazer o vínculo/criação do cliente
  clienteTelefone: z3.string().min(8, "Telefone \xE9 obrigat\xF3rio"),
  // CPF e Email continuam opcionais com preprocessamento
  clienteEmail: z3.preprocess(
    (val) => val === "" ? void 0 : val,
    z3.string().email("E-mail inv\xE1lido").optional().nullable()
  ),
  clienteCpf: z3.preprocess(
    (val) => val === "" ? void 0 : val,
    z3.string().optional().nullable()
  ),
  // --- DADOS DO PROCESSO ---
  descricaoObjeto: z3.string(),
  numeroInterno: z3.string().optional(),
  numeroProcesso: z3.preprocess(
    (val) => val === "" ? void 0 : val,
    z3.string().optional().nullable()
  ),
  responsavel: z3.enum(["Leonardo", "Alberto", "Jenifer"]),
  tipoHonorarios: z3.enum(["Iniciais", "\xCAxito", "Ambos"]),
  valorPrevistoIniciais: z3.number().default(0),
  valorPrevistoExito: z3.number().default(0),
  basePrevisao: z3.string().optional().nullable(),
  dataEstimadaRecebimento: z3.preprocess(
    (val) => val === "" ? null : val,
    // Trata string vazia como null
    z3.coerce.date().optional().nullable()
    // Aceita string ISO e vira Date
  ),
  custosPrevistos: z3.number().default(0),
  dataFechamentoContrato: z3.coerce.date(),
  // Aceita string ISO
  statusProtocolamento: z3.string().default("Pendente de Protocolamento"),
  statusGeral: z3.string().default("Contrato Fechado"),
  // --- NOVO: ARQUIVOS ---
  arquivos: z3.array(z3.object({
    tipo: z3.string(),
    url: z3.string(),
    nomeArquivo: z3.string()
  })).optional().default([])
});
...
var atualizarEtapaSchema = z5.object({}).catchall(z5.any());
...
var DocumentAnalysisService = class {
  async analyzeDocument(fileBuffer, docTypeContext) {
    ...
    try {
      const extractionSchema = z6.object({
        tipo_identificado: z6.enum(["RG", "CNH", "COMPROVANTE_RESIDENCIA", "OUTROS"]),
        nome_completo: z6.string().optional(),
        rg_numero: z6.string().optional().describe("N\xFAmero do Registro Geral (RG) ou n\xBA de registro da CNH"),
        cpf_numero: z6.string().optional().describe("N\xFAmero do CPF (Cadastro de Pessoa F\xEDsica). Formato: xxx.xxx.xxx-xx"),
        endereco_completo: z6.string().optional(),
        data_emissao: z6.string().optional(),
        legivel: z6.boolean().describe("Se o documento est\xE1 n\xEDtido e leg\xEDvel")
      });
...
var ClientesController = class {
  ...
  async update(req, rep) {
    ...
    const bodySchema = z7.object({
      nome: z7.string().optional(),
      email: z7.string().email().optional().nullable(),
      cpf: z7.string().optional().nullable(),
      telefone: z7.string().optional(),
      endereco: z7.string().optional().nullable()
    });
    const { id } = paramsSchema.parse(req.params);
...
var createFinanceiroSchema = z8.object({
  // MUDANÇA AQUI: Padronizado para minúsculo
  tipo: z8.enum(["entrada", "saida"]),
  categoria: z8.string(),
  valor: z8.number().positive(),
  data: z8.coerce.date(),
  descricao: z8.string().min(3),
  // Aceita string (ID do Mongo), null ou undefined
  processoId: z8.string().optional().nullable(),
  // Garante que o campo exista no objeto final 'data'
  recorrente: z8.boolean().default(false)
});
...
app.get("/api/cron/notify-agenda", async (req, reply) => {
  ...
  const notifyService = new NotifyDailyAgendaService(mailService2);
...
if (process.env.NODE_ENV !== "production") {
  const start = async () => {
    try {
      ...
      console.log("\u{1F680} RCS Advogados - Rodando localmente na porta 3333");
    } catch (err) {
      app.log.error(err);
      process.exit(1);
    }
